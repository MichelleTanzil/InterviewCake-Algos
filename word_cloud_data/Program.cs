...
namespace word_cloud_data
{
    class Program
    {
        private Dictionary<string, int>
            _wordsToCounts = new Dictionary<string, int>();

        public IDictionary<string, int> WordsToCounts
        {
            get
            {
                return _wordsToCounts;
            }
        }
            ...
            for (int i = 0; i < inputString.Length; i++)
            {
                ...
                if (i == inputString.Length - 1)
                {
                    ...
                    if (currentWordLength > 0)
                    {
                        var currentWord =
                            inputString
                                .Substring(currentWordStartIndex,
                                currentWordLength);
        ...
        private void AddWordToDictionary(string word)
        {
            ...
            if (_wordsToCounts.TryGetValue(word, out currentCount))
            {
                ...
                _wordsToCounts.TryGetValue(word.ToLower(), out currentCount)
            )
            {
                ...
                _wordsToCounts.TryGetValue(Capitalize(word), out currentCount)
            )
            {
